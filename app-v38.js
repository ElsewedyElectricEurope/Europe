...
/* Plain scrollIntoView({block:"start"}) lines a section's top edge up with
   y=0 of the viewport — but the sticky header sits right on top of that,
   covering exactly the "Partners in X ... Projects won" heading we most
   want visible. This scrolls to the section's real top, offset by the
   sticky header's own height (plus a little breathing room), so the
   heading always lands just below the header instead of behind it. */
...
function clearSelection(){
  ...
  renderPartnersPanel();
  // Deselecting (re-clicking the same country, or hitting "Clear selection")
  // takes the user back up to the flag grid so they can pick another country.
...
/* country name labels — drawn in a second pass so every label sits on top of
   all fills. Only the 25 partner (red) countries get a label at all now —
   the 12 non-partner (gray) countries are left unlabeled entirely, per
   request, which also freed up a lot of room: every label is now inline,
   directly on its own shape, with no leader lines/dots/external margin text.
   LABEL_STYLE holds a per-country [fontSize, dx, dy], unified into three
   readable tiers (9.5 / 8.3 / 7.3px) based on each country's real on-map
   footprint, plus one smaller size (6.3px) for Albania — a genuinely very
   narrow sliver of a shape (~19 units wide) where even the smallest tier
   would overflow its own outline a little; that's an unavoidable tradeoff
   of keeping every label inline rather than moved out to a margin. dx/dy
   nudges only exist where two neighboring labels would otherwise touch
   (Finland's baked-in centroid sits oddly high in its shape; Ireland needed
   a small nudge to stop brushing the UK's much longer "United Kingdom").
   Re-verified zero label-vs-label overlaps and everything inside the
   viewBox after this change. */
const LABEL_STYLE = {
  DEU:[9.5,0,0], ESP:[9.5,0,0], FRA:[9.5,0,0], ITA:[9.5,0,0],
  GBR:[9.5,0,0], POL:[9.5,0,0], NOR:[9.5,0,0], GRC:[9.5,0,0],
  ROU:[9.5,0,0], FIN:[9.5,4.9,17.7],
  AUT:[8.3,0,0], BGR:[8.3,0,0], CZE:[8.3,0,0], HRV:[8.3,0,0],
  HUN:[8.3,0,0], CHE:[8.3,0,0], DNK:[8.3,0,0], LTU:[8.3,0,0],
  LVA:[8.3,0,0], BEL:[8.3,0,0],
  NLD:[7.3,0,0], IRL:[7.3,-4.0,0], PRT:[7.3,0,0], SVN:[7.3,0,0],
  ALB:[6.3,0,0]
};
/* Every labeled (partner) country now shares the exact same text color and
   font — no more per-country black-text exceptions — so the map reads as
   one consistent, unified style rather than a mix. */
Object.keys(COUNTRY_PATHS).forEach(id=>{
  const isPartner = !!(A3_TO_A2[id] && COUNTRY_DATA[A3_TO_A2[id]]);
  if(!isPartner) return; // non-partner countries stay unlabeled
  const c = COUNTRY_PATHS[id];
  const style = LABEL_STYLE[id];
  if(!style) return;
  const label = document.createElementNS(svgNS, "text");
  label.setAttribute("x", c.cx + style[1]);
  label.setAttribute("y", c.cy + style[2]);
  label.setAttribute("class", "country-label partner");
  label.setAttribute("text-anchor", "middle");
  label.setAttribute("dominant-baseline", "middle");
  label.style.fontSize = style[0] + "px";
