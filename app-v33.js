...
Object.keys(COUNTRY_PATHS).forEach(id=>{
  ...
  /* no native <title> child here on purpose — every country already has a
     permanent visible text label on the map, so an SVG <title> would only
     add an unstyled, inconsistent native browser tooltip on top of it (and
     on top of the custom tooltip for partner countries). */
...
/* country name labels — drawn in a second pass so every label sits on top of all fills.
   LABEL_STYLE holds a per-country [fontSize, dx, dy]: fontSize is sized to the
   country's own on-map footprint (computed from its real path geometry, so a
   small country like Luxembourg gets much smaller text than France), and
   dx/dy are small manual offsets — solved so that no two labels' bounding
   boxes touch — for the handful of countries in dense clusters (Balkans,
   Ireland/UK) where even a right-sized label would otherwise brush a
   neighbor's text. Together this keeps every label inside (or right at the
   edge of) its own shape without ever overlapping another label.
   (GBR's dx/dy is 0 here, unlike the original version of this table — this
   map now uses country-paths-v2.js, whose GBR/NOR/GRC/HRV anchor points were
   corrected to their true polygon centroids, so the old manual nudge is no
   longer needed and would just overshoot.) */
const LABEL_STYLE = {
  ALB:[4.2,-0.2,0.1], AUT:[9.0,0,0], BEL:[7.7,0,0], BGR:[9.0,0,0],
  BIH:[3.6,-3.9,-0.6], BLR:[7.4,0,0], CHE:[5.9,0,0], CZE:[7.0,0,0],
  DEU:[9.0,0,0], DNK:[6.0,0,0], ESP:[9.0,0,0],
  FIN:[9.0,4.9,17.7], FRA:[9.0,0,0], GBR:[8.3,0,0], GRC:[9.0,0,0],
  HRV:[9.0,0,0], HUN:[9.0,0,0], ISL:[7.4,0,0],
  ITA:[9.0,0,0], LUX:[3.6,0,0],
  MDA:[7.4,0,0], MKD:[3.6,0.6,-0.3], MNE:[3.6,0,0], NLD:[5.1,0,0],
  NOR:[9.0,0,0], POL:[9.0,0,0], PRT:[5.8,0,0], ROU:[9.0,0,0],
  SRB:[7.4,3.9,0.6], SVK:[7.4,0,0], SVN:[5.3,0,0], SWE:[7.4,0,0],
  UKR:[7.4,0,0]
};
/* Ireland and the three Baltic states are too small/cramped for readable
   inline text at this scale, so — matching the reference — they instead get
   a small dot on the country, a thin leader line, and their name set in the
   open space just outside the map's own edge (there's already plenty of
   native margin there in the original 980x720 viewBox, so no resizing is
   needed). Label Y = the country's real centroid Y, same as its dot, so
   every line comes out perfectly horizontal. */
const EXTERNAL_LEFT = [["IRL", 351.3]];
const EXTERNAL_RIGHT = [["EST", 260.7], ["LVA", 291.9], ["LTU", 322.1]];
const LEFT_TEXT_X = 80, RIGHT_TEXT_X = 615;
const EXTERNAL_IDS = new Set([...EXTERNAL_LEFT, ...EXTERNAL_RIGHT].map(([id])=>id));
/* A few red (partner) countries read better with black text than the usual
   white-on-red — a manual per-country exception rather than a blanket rule,
   since every other partner country keeps white text. */
const FORCE_DARK_TEXT = new Set(["NOR", "IRL", "LVA", "LTU"]);

Object.keys(COUNTRY_PATHS).forEach(id=>{
  if(EXTERNAL_IDS.has(id)) return; // drawn separately below
  const c = COUNTRY_PATHS[id];
  const isPartner = !!(A3_TO_A2[id] && COUNTRY_DATA[A3_TO_A2[id]]);
  const style = LABEL_STYLE[id] || [null, 0, 0];
  const label = document.createElementNS(svgNS, "text");
  label.setAttribute("x", c.cx + style[1]);
  label.setAttribute("y", c.cy + style[2]);
  label.setAttribute("class", "country-label " + (isPartner ? "partner" : "") + (FORCE_DARK_TEXT.has(id) ? " dark-text" : ""));
  label.setAttribute("text-anchor", "middle");
  label.setAttribute("dominant-baseline", "middle");
  if(style[0]) label.style.fontSize = style[0] + "px";
  label.textContent = c.name;
  g.appendChild(label);
});

function drawExternalLabel(id, labelY, side){
  const c = COUNTRY_PATHS[id];
  const isPartner = !!(A3_TO_A2[id] && COUNTRY_DATA[A3_TO_A2[id]]);
  const textX = side === "left" ? LEFT_TEXT_X : RIGHT_TEXT_X;
  const lineEndX = side === "left" ? textX + 8 : textX - 8;
  ...
  label.setAttribute("class", "country-label external" + (isPartner ? " partner" : "") + (FORCE_DARK_TEXT.has(id) ? " dark-text" : ""));
  label.setAttribute("text-anchor", side === "left" ? "end" : "start");
  label.setAttribute("dominant-baseline", "middle");
  label.textContent = c.name;
  g.appendChild(label);
}
EXTERNAL_LEFT.forEach(([id,y])=> drawExternalLabel(id, y, "left"));
EXTERNAL_RIGHT.forEach(([id,y])=> drawExternalLabel(id, y, "right"));
